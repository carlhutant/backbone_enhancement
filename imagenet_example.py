...
def main():
    ...
    main_worker(configure.specified_GPU_ID, ngpus_per_node, args)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    print("=> creating model '{}'".format(configure.model1))
    model = models.__dict__[configure.model1]()
    ...
    #         model = torch.nn.parallel.DistributedDataParallel(model)
        ...
        if configure.model1.startswith('alexnet') or configure.model1.startswith('vgg'):
    ...
    if configure.resume_ckpt_path:
        if os.path.isfile(configure.resume_ckpt_path):
            print("=> loading checkpoint '{}'".format(configure.resume_ckpt_path))
            if args.gpu is None:
                checkpoint = torch.load(configure.resume_ckpt_path)
            else:
                # Map model to be loaded to specified single gpu.
                loc = 'cuda:{}'.format(args.gpu)
                checkpoint = torch.load(configure.resume_ckpt_path, map_location=loc)
            ...
            print("=> loaded checkpoint '{}' (epoch {})"
                  .format(configure.resume_ckpt_path, checkpoint['epoch']))
        else:
    ...
    for epoch in range(args.start_epoch, configure.epochs):
        # if args.distributed:
